...
| `hail_tsv` | `false` | Produce a TSV formatted for the Hail WxS-QC pipeline. **Requires `left_align = true`.** |
| `left_align` | `false` | Left-align indels (via `bcftools norm -f ref_fasta`) before annotating. Required for `hail_tsv`. Requires `ref_fasta` to point to a real file. |
...
| `hail_tsv = true` (+ `left_align = true`) | `WxS_QC_CSQ.tsv.gz` + `.tbi` — one combined file across all inputs |
...
| `transcript_mode` | `all` \| `worst` \| `primary` \| `mane` — which transcript(s) VEP reports per variant |
| `annotate_vcf` / `csq_tsv` / `hail_tsv` / `left_align` | Run modes, see above |
...
- `left_align = true` is required whenever `hail_tsv = true`; the pipeline will fail fast with a clear error if you enable one without the other.
